...
import { UserProfile, PLANS, PlanDetails, PlanType, CREDIT_COSTS, SubscriptionDetails } from '../types';
import { Check, X, Zap, Crown, LayoutDashboard, CreditCard, BarChart3 } from 'lucide-react';
...
type TabType = 'overview' | 'subscription';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#0f172a] border border-slate-700 w-full max-w-5xl h-[90vh] overflow-y-auto rounded-3xl shadow-2xl flex flex-col relative">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-[#0f172a]/95 backdrop-blur border-b border-slate-800">
            ...
            <>
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6 relative overflow-hidden group">
                  ...
                  <div className="mt-4 w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.min(100, (userProfile.credits / 1000) * 100)}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {userProfile.plan === 'free' && (
                      <span className="px-2 py-1 bg-yellow-500/20 text-yellow-500 text-xs font-bold rounded uppercase">
                        Trial
                      </span>
                    )}
                    {subscription.cancelAtPeriodEnd && userProfile.plan !== 'free' && (
              ...
              <div className="bg-indigo-900/10 border border-indigo-500/20 rounded-xl p-4 flex flex-wrap gap-6 items-center justify-center text-sm text-indigo-200">
                <span className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-indigo-500 rounded-full"></div> 1 Thumbnail (Standard) ={' '}
                  {CREDIT_COSTS.THUMBNAIL_STANDARD} Credits
                </span>
                <span className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-purple-500 rounded-full"></div> 1 Video ={' '}
